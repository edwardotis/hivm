...
//TODO create cache directory if needed
...
bool BioProcessor::load_cache( const PreProcWUSet& train_input, SvmWUMatrix& train_output )
{
			...
			tok_iter_row++, cur_row++ )
	{
		...
		SvmWUSet* svmset_train_out;
		try
		{
			svmset_train_out = new SvmWUSet( 
		...
		{
			std::string msg = "\nException caught in BioProcessor::load_cache( train_output ) new SvmWUSet \n";
			msg +=   e.what();
			msg += "\n\nElapsed Time: ";
...
bool BioProcessor::load_cache( const PreProcWUSet& train_input, const PreProcWUSet& predictee_input, SvmWUMatrix& predictees_output )
{
			...
			tok_iter_row++, cur_row++ )
	{
		...
		SvmWUSet* svmset_predictee_out;
		try
		{
			svmset_predictee_out = new SvmWUSet( 
		...
		{
			std::string msg = "\nException caught in BioProcessor::load_cache( predictees_output ) new SvmWUSet \n";
			msg +=   e.what();
			msg += "\n\nElapsed Time: ";
			msg += Log::elapsed_time();
			msg += "\nExiting...";
		...
		)
{
	...
	for( int outer = 0; outer < predictee_input.size(); outer++ )
	{
		...
		try
		{
			wu_set = new SvmWUSet( predictee_input[outer]->get_id(), 
		...
		{
			std::string msg = "\nException caught in BioProcessor::process_predictees call to new \n";
			msg +=   e.what();
			msg += "\n\nElapsed Time: ";
			msg += Log::elapsed_time();
			msg += "\nExiting...";
