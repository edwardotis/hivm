...
void Options::_initialize( int argc, char* argv[] )
{
	...
	if( thresholds.empty() )
	{
		thresholds.push_back( threshold_default );
	}
...
bool Options::is_seed_used_() const
{
	if( purpose == "model-selection" )
	{
		if( use_entire_susceptbility_file == false )
		{
			return true;
		}
	}
...
void Options::save_cmd_line_options_( const std::string file_name_prefix  ) const
{
	...
	else if( purpose == "model-validation" )// i.e. test
	{
		opts.append(    "--purpose       model-validation\n" );
		cmdline.append( "--purpose       model-validation " );	
	}
	...
	//common to all
	if( purpose == "model-selection" ||  purpose == "model-validation" )
	{
		...
		{
			opts.append( "--seed          " + boost::lexical_cast<std::string>( seed ) + " IGNORED!\n" );
		}
	...
	if( purpose == "model-validation" )
	{
		...
		opts.append( "--test-dataset  " + boost::lexical_cast<std::string>( susceptibility_file_test_dataset ) + "\n" );
		...
		cmdline.append( "--test-dataset  " + boost::lexical_cast<std::string>( susceptibility_file_test_dataset ) + " " );
	}
...
//output to std::err
	#if defined UI_TEST_UNATTENDED || !defined  UNIT_TESTING 
		std::cerr << opts << '\n';
	#endif
