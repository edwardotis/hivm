...
void FileIO::read( const std::string path, std::string& input)
{
	...
	//TODO test for crap line getting added in PR files
	//how do we know if there's an extra line or not?

	//remove extra line break I added to end of input stream
	//size_t end = temp_str.end();
	//size_t index = temp_str.find_last_of( '\n' );
	//std::cout << temp_str;
	//temp_str.erase( temp_str.begin()+index  );
	//std::cout << temp_str;
	//input.append( temp_str );
	//input.append( boost::algorithm::trim_right_copy( temp_str ) );
//	std::cout << temp_str;
...
//fs::error_code FileIO::_errorCheck( fs::path )
//{
//
//	return fs::no_error;
//}

//bool fileio::read_( const boost::filesystem::path,  stringstream& output )
//{
//	ifstream infile;
//	
//	if( infile.is_open() ){	
