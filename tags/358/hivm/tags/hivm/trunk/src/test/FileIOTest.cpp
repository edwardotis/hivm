...
BOOST_AUTO_TEST_CASE( FileIOTest_write_create_new_directories_if_needed )
{
	...
	//if( fs::exists( f.path_ ) )
	//{
	//	fs::remove( f.path_ );//remove file
	//}

	//std::string str_path2 = "../data/TestFileIO/" + rand_path1 + "/" + rand_path2;


	//just testing these functions out.
	//need a try/catch block when using exist, isempty, isdirectory
	//if( fs::exists( str_path2 ) && 
	//	fs::is_empty( str_path2 ) &&
	//	fs::is_directory( "dfd" ) 
	//  )
	//{
	//	fs::remove( str_path2 );//dir2
	//}

	//fs::remove( str_path1 );//dir1


	//while( fs::exists( p.branch_path() ) )
	//{
	//	fs::remove( p.branch_path() );
	//	std::string restofpath =  p.branch_path().string();
	//	fs::path p2 (restofpath);
	//	p = p2;
	//}
}
