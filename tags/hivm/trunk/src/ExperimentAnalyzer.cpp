...
std::vector<std::string> ExperimentAnalyzer::get_ids( const ExperimentResult* result )
{
	...
	for( size_t i=0; i < result->datum.data_points.size(); i++ )
	{
		ids.push_back( result->datum.data_points.at(i)->id );
	}
	return ids;
}
				...
				const std::string output_prefix )
{
	...
	for(size_t i=0; i < result->datum.data_points.size(); i++ )
	{
		output.append( result->datum.data_points.at(i)->id + '\n' );
	}
...
void ExperimentAnalyzer::set_tp_tn_fn_fp_n( const ExperimentResult* result )
{
	...
	//N
	this->N = result->datum.data_points.size();
	
	for( size_t i = 0; i < result->datum.data_points.size(); i++ )
	{
		if( result->datum.data_points.at(i)->predicted_susceptibility == true)
		{
			if(	result->datum.data_points.at(i)->actual_susceptibility  == true )
			{
				TP++;
			}
			if(	result->datum.data_points.at(i)->actual_susceptibility  == false )
			{
				FP++;
			}
		}

		if( result->datum.data_points.at(i)->predicted_susceptibility == false)
		{
			if(	result->datum.data_points.at(i)->actual_susceptibility  == true )
			{
				FN++;
