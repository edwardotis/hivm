...
size_t PreProcessor::calc_number_columns( const std::string line )
{
	//find end of first row

	//boost::numeric_cast<size_t>(first_nl_pos) = input.find( '\n' );
...
bool PreProcessor::is_correct_num_columns( std::string line, size_t num_header_columns )
{
	size_t cur_col_count =	calc_number_columns( line );
...
std::vector<size_t> PreProcessor::screen_drug ( const std::string drug, const string_spread_sheet& spread_sheet )
{
	std::vector<size_t> rows_to_remove;

	//find header column
	size_t drug_fold_col = find_drug_fold_col( drug, spread_sheet );

	for( size_t row = 1; row < spread_sheet.size(); row++ )//skip header row
	{
...
size_t PreProcessor::find_drug_fold_col( const std::string drug, const string_spread_sheet& spread_sheet )
{
...
size_t PreProcessor::find_header_col( const std::string header_name, const string_spread_sheet& spread_sheet )
{
...
bool PreProcessor::is_invalid_drug_fold( const spread_sheet_row_const_iterator& row_it, size_t drug_fold_column )
{
	bool invalid = true;
	bool valid   = false;
...
bool PreProcessor::is_invalid_drug_foldmatch( const spread_sheet_row_const_iterator& row_it, size_t drug_foldmatch_column )
{
	bool invalid = true;
	bool valid   = false;
...
void PreProcessor::erase_rows( string_spread_sheet& spread_sheet, std::vector<size_t>& rows_to_delete )
{
	...
	for( size_t row_index = 1; row_index < spread_sheet.size(); row_index++ )
	{
	...
	//size_t dropped = spread_sheet.size() - rows_kept_counter;
...
std::string PreProcessor::create_mutation_string( const std::string wild_sequence, const string_spread_sheet_row& row)
{
	...
	size_t index_offest = 0;//can be negative or positive
	
	for( size_t pos = 0; pos < mut_vector.size(); pos++ )
	{
...
//DEPRECATED
bool PreProcessor::is_susceptible( const size_t drug_column, const string_spread_sheet_row& row,  double threshold )
{
...
bool PreProcessor::is_susceptible( const size_t drug_column, const string_spread_sheet_row& row, const std::vector<double> thresholds )
{
	bool susceptible = true;
	bool not_susceptible = false;
...
//	for( size_t row = 1; row < spread_sheet.size(); row++ )
//	{
//		//get data for WU
//		//id
...
//screen
std::vector<size_t> rows_to_remove_indices = this->screen_drug( drug, spread_sheet );
remove_duplicate_entries( drug, wild_sequence_path, spread_sheet, rows_to_remove_indices );
screen_type( drug, spread_sheet, preproc_options.susceptibility_type, rows_to_remove_indices );

size_t fold_col = this->find_drug_fold_col( drug, spread_sheet );
										...
										std::list<size_t>&  rows_for_testing_indices )
{
	...
	for( size_t i=1; i < spread_sheet.size(); i++ )//skip header index
	{	
	...
	std::vector<size_t>& rows_to_remove )
{
	...
	size_t num_duplicates_removed = 0;

	//for each row in spreadsheet
	for( size_t base_row=1; base_row < spread_sheet.size(); base_row++ )//skip header row
	{
		//if( base_row is NOT already set for removal)
		if( std::count( rows_to_remove.begin(), rows_to_remove.end(), base_row ) == 0 )
		{
			...
			for( size_t test_row=1; test_row < spread_sheet.size(); test_row++ )//skip header row
			{
...
void PreProcessor::update_usable_input_indices( const std::vector<size_t>& rows_to_remove_indices, 
								 std::list<size_t>& output_indices )
{
	std::vector<size_t>::const_iterator it;
	for( it=rows_to_remove_indices.begin(); it != rows_to_remove_indices.end(); it++ )
	{
		size_t count = std::count( output_indices.begin(), output_indices.end(), *it );
		if( count > 1)//error
		{
			std::string msg = "Error splitting of training and testing input data sets.\n";
...
{
	//find header column
	size_t type_col = find_header_col( "Type", spread_sheet );
