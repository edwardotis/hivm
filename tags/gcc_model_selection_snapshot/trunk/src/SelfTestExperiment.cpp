...
void SelfTestExperiment::run_experiment( const PreProcWUSet& input, 
				...
				const Options& svm_param_options )
{
	...
	//}
	//return run_experiment_( input, result, svm_param_options, lg_cost, lg_gamma );
}
...
//	int default_lg_cost = 0;
//	int default_gamma_cost = 0;
//
//	return run_experiment_( input, result, svm_param_options, default_lg_cost, default_gamma_cost );
//}
...
//
//	LibSvmAdapter l;
//	std::vector<bool> susceptibility_predicted_results = l.predict_susceptibility( svm_trainset, svm_predictees, svm_param_options, lg_cost, lg_gamma );
//
//	//Collect each Result here:
//	for( int i=0; i < susceptibility_predicted_results.size(); i++ )
//	{
...
//	}
	...
	//calculate results
	//int hits = 0;
	//for( int i = 0; i < svm_predictees.size(); i++ )
	//{
	...
	//int num_total_predictions = svm_predictees.size();
