...
void ExperimentAnalyzer::prune( const ExperimentResultSet& input, ExperimentResultSet& output )
{
	...
	//int best_tpr_index = 0;
	//for( int i=0; i < input_copy.size()-1; i++ )//skip front
	//{
...
{
	std::vector<double> probabilities = result->data.data_points.at(it)->probabilities;
	if( probabilities.size() == 0 )//probability data was not collected
	{
		return "n/a";
	}

	if( result->data.data_points.at(it)->predicted_susceptibility == true )
	{
	...
	const std::string output_prefix )
{
	...
	//create header row
	output.append( "Isolate, Predicted Susceptible? (0|1), Probability_of_Pred., Lg_Cost, Lg_Gamma\n" );

	//enter data
	for(int i=0; i < result->data.data_points.size(); i++ )
	{
		output.append( result->data.data_points.at(i)->id + ", " );
		output.append( boost::lexical_cast<std::string>( result->data.data_points.at(i)->predicted_susceptibility ) + ", " );
	...
	/*
			if( result->data.data_points.at(i)->predicted_susceptibility == 1 )
		{
			output.append( "yes"
		}
	*/
				...
				const std::string output_prefix )
{
	...
	{
		output.append( result->data.data_points.at(i)->id + '\n' );
		//output.append( result->data.data_points.at(i)->actual_susceptibility + '\n' );
	}
...
{	
	...
	Log::append( "Number of data points collected per Experiment: " + boost::lexical_cast<std::string>( ( result_set.front()->data.data_points.size() ) ) );
	Log::append( "Total data points collected: " + boost::lexical_cast<std::string>( ( result_set.size() * result_set.front()->data.data_points.size() ) ) );
	...
	if( options.purpose == "model-selection" || options.purpose == "self-validate" )
	{
