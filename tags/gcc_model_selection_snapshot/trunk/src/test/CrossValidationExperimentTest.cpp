...
BOOST_AUTO_TEST_CASE( CrossValidationExperimentTest_create_fold_pr_small23_NFV_fold0 )
{
	...
	//sizes
	int total_size = trainset_output.size() + predictees_output.size();
	...
	//test trainset
	int remainder = 3;
	for( int i = 0; i < f.input_set.size()-remainder; i++ )
	{
...
BOOST_AUTO_TEST_CASE( CrossValidationExperimentTest_create_fold_pr_small23_NFV_fold3 )
{
	...
	for( int i = 11; i < f.input_set.size(); i++ )
	{
...
BOOST_AUTO_TEST_CASE( CrossValidationExperimentTest_create_fold_pr_small23_NFV_fold2 )
{
	...
	for( int i = 9; i < f.input_set.size(); i++ )
	{
...
BOOST_AUTO_TEST_CASE( CrossValidationExperimentTest_pr50_NFV_single_cg_pair )
{
	...
	CrossValidationExperiment cve;
	cve.run_experiment(f.input_set, result, o );
	...
	BOOST_CHECK_EQUAL( num_data_points, results.front()->data.data_points.size() );
	...
	for( int i=0; i < f.input_set.size(); i++ )
	{
		BOOST_CHECK_EQUAL( f.input_set.at(i)->known_susceptibility(),
			results.front()->data.data_points.at(i)->actual_susceptibility );
	}
...
BOOST_AUTO_TEST_CASE( ValidationExperimentTest_pr50_NFV_single_cg_pair )
{
	...
	std::string drug = "NFV";
	double threshold = 10;//should be median of the file
	int seed = 42;//seed for randomly splitting test and training data sets
	...
	//Tests
	BOOST_CHECK_EQUAL( results.front()->lg_cost, 1 );
	BOOST_CHECK_EQUAL( results.front()->lg_gamma, 1 );
