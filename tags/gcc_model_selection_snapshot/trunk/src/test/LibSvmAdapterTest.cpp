...
BOOST_AUTO_TEST_CASE(LibSvmAdapterTest_number_of_classes)
{
...
BOOST_AUTO_TEST_CASE( LibSvmAdapterTest_predict_collection_1class_predictions_short )
{
	...
	//Evaluates to 1 class
	{
		...
		BOOST_CHECK_EQUAL( result->data.data_points.size(), svm_predictee_matrix.size() );

		for( int i = 0; i < result->data.data_points.size(); i++ )
		{
			bool known = svm_train_matrix.front()->known_susceptibility();
			bool predicted = result->data.data_points.at(i)->predicted_susceptibility;
...
{
	//LibSvmAdapterTest_Fixture f( "../data/PreProcessorTest/PR_2006-05-25_v2.0_small2.tsv" );
	//LibSvmAdapterTest_Fixture f( "../data/ExperimentAnalyzerTest/PR_2006-05-25_v2.0_NFV25.tsv" );
	...
	Log::append( "LibSvmAdapterTest_Fixture svm_predictee_matrix.size = " +  boost::lexical_cast<std::string>( svm_predictee_matrix.size() )  );
	...
	//for( int e=0; e < result->data.data_points.size(); e++ )
	//{
	//	id = result->data.data_points.at(e)->id;
	//	suscep = result->data.data_points.at(e)->predicted_susceptibility;
	//	std::cout << id <<  " " << suscep  << '\n';
	//}
	...
	*/
	...
	for( int i = 0; i < result->data.data_points.size(); i++ )
	{
		...
		{
			if( result->data.data_points.at(i)->predicted_susceptibility == 1 )//susceptible
			{
...
//	std::vector<bool> results;
	...
	if( svm_result == -1 )
	{
		...
		BOOST_CHECK_EQUAL( is_libsvm_susceptible, result->data.data_points.front()->predicted_susceptibility );
	}
...
BOOST_AUTO_TEST_CASE( LibSvmAdapterTest_predict_with_probability_single_pr_small3_probabilty_collection_order )
{
	...
	*/
	...
	for( int i = 0; i < result->data.data_points.size(); i++ )
	{
		std::vector<double> probabilities = result->data.data_points.at(i)->probabilities;
