...
void SvmMachine::run( const std::string purpose, const Options& options  )
{
...
void SvmMachine::cross_validate( const Options& options )
{
	//initialize
...
void SvmMachine::self_validate( const Options& options  )
{
	//initialize
	...
	)
{
	...
	for( c = log_cost_low; c <= log_cost_high; c+=log_cost_increment )
	{
		...
		for( g = log_gamma_low; g <= log_gamma_high; g+=log_gamma_increment )
		{
			...
			//run experiment
			experiment.run_experiment(input_data, result, svm_param_options, c, g ); 
