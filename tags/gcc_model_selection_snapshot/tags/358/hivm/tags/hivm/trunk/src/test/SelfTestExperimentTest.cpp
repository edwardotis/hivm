...
//		result->datum.data_points.push_back( point );
//
//		BOOST_CHECK_EQUAL( result->log_cost, -13 );
//		BOOST_CHECK_EQUAL( result->log_gamma, 3 );
//		BOOST_CHECK_EQUAL( result->datum.data_points.front()->predicted_susceptibility, true );
//		BOOST_CHECK_EQUAL( result->datum.data_points.front()->actual_susceptibility, false );
//	
//		delete result;
//	}
//}
...
BOOST_AUTO_TEST_CASE( SelfTestExperimentTest_run_experiment_pr_small23 )
{
	...
	Options o;
	ste.run_experiment( f.input_set, er, o, 0, 0 );

	//check out ExperimentResult more here.

	BOOST_CHECK_EQUAL( er->log_cost,  2 );
	BOOST_CHECK_EQUAL( er->log_gamma, 2  );
	...
	{
		BOOST_CHECK_EQUAL( er->datum.data_points.at(i)->actual_susceptibility, f.input_set.at(i)->known_susceptibility() );
	}
...
BOOST_AUTO_TEST_CASE( SelfTestExperimentTest_pr_NFV25_t10 )
{
...

	{
		...
		ste.run_experiment( f.input_set, er, o, log_cost, log_gamma );

		//analyze
		ExperimentAnalyzer ea;
		double test_accuracy = ea.accuracy( er );
