...
BOOST_AUTO_TEST_CASE( boost_tests_matrices )
{
	...
	//OK, head hurts. maybe get pencil and paper.
	// matrix is supposed to ease use.
	// test a matrix of shared_ptrs of strings
	// or shared_ptr of a matrix of strings. (does this even make sense?) No.
	// in fact, strings destruct themselves quote nicely. never call new() on them

	// I'm jumping ahead to the point of WorkUnit matrix for computing Local Alignment.
	//that will be a matrix of shared ptrs to WU's. and I'll call string function of them
	// by using boost::bind, so that I can use std::algorithms, like for_each

	//typedef boost::shared_ptr< std::string > string_ptr;
	////typedef nu::matrix< std::string > container;
	//nu::matrix< string_ptr > _m4(2,3);
	//_m4(0,0) = (new std::string("hello world"));

	//BOOST_CHECK_EQUAL( _m4(0,0).get->c_str(), "hola world" );
}
